...
#definindo as colunas que serão utilizadas na análise
selected_columns <- c("ListingCreationDate","Term","LoanStatus","ClosedDate",
                      "BorrowerRate", "LenderYield","EstimatedReturn",
                      "ProsperScore", "ListingCategory","BorrowerState", 
                      "Occupation","EmploymentStatus","EmploymentStatusDuration",
                      "IsBorrowerHomeowner", "FirstRecordedCreditLine",
                      "BankcardUtilization","AvailableBankcardCredit",
                      "TradesOpenedLast6Months","StatedMonthlyIncome",
                      ...
                      "Recommendations", "IncomeRange", "ProsperRating")
...
selected_loans$LoanOriginationDate <- as.character(
                                             selected_loans$LoanOriginationDate)
#convertendo para data
selected_loans$LoanOriginationDate <- as.Date(
                                            selected_loans$LoanOriginationDate,
                                            format= "%Y-%m-%d")
#obtendo o ano da data correspondente
...
#Responsável por exibir o valor dos empréstimos com todos os dados
...
#Responsável por exibir os dados limitando o eixo X
ggplot(aes(x = LoanOriginalAmount), data = selected_loans) +
  geom_histogram() +
  ...
  labs(title="Valores dos empréstimos até 25 mil dolares", 
     x = "Valor do Empréstimo",
     y = "Número de registros")
#Exibe o sumário do valor do empréstimo
...
#cria um agrupamento pela faixa de renda
selected_loans.fc_by_IncoreRange <- selected_loans %>%
  group_by(IncomeRange) %>%
  summarise(n = n()) 

#labels dos ranges na ordem que serão exibidos
ranges <-c("$0", "$1-24,999", "$25,000-49,999", "$50,000-74,999",
           "$75,000-99,999","$100,000+","Not employed", "Not displayed")

#Responsável por exibir a distribuição da faixa de renda
  ggplot(selected_loans.fc_by_IncoreRange, 
         aes(x = IncomeRange, y = n, label = n)) +
  ...
  theme(legend.position = "bottom", 
    ...
    labs(title="Distribuição da faixa de renda", 
     ...
     x = "Faixa de Renda")
#Exibe os dados da faixa de renda em uma tabela
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Responsável por exibir a distribuição da renda mensal dos mutuários.
ggplot(aes(x = StatedMonthlyIncome), data = selected_loans) +
  geom_histogram() +
  scale_x_continuous(limits = c(0, quantile(
                                    selected_loans$StatedMonthlyIncome, 0.99)),
                     breaks = seq(0, 
                                  quantile(
                                    selected_loans$StatedMonthlyIncome, 0.99), 
                                    2500)) +
...
```
Como já previa, a distribuição da renda mensal possui uma assimetria positiva, com uma concentração próxima de $ 5.000. Para analisar melhor a distribuição, decidi exclui 1% dos dados superiores, mas, antes disso, descobri que um mutuário com uma renda mensal de quase 2 milhões de dolares, solicitou um empréstimo de 4 mil dolares. Acredito que esse mutuário estava interessado em participar como investidor e fez um empréstimo baixo para verificar a lisura do funcionamento da Prosper.




```{r echo=FALSE, message=FALSE, warning=FALSE}
#Responsável por exibir um gráfico de barras das situações dos empréstimos
...
table(selected_loans$LoanStatus)
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Responsável por exibir um histograma dos scores de risco dos empréstimos
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#criando a variável inadimplente que indica quando um mutuário está inadimplente
# de acordo com o status do empréstimo
...
selected_loans$stateFullName <- tolower(
                                        state.name[
                                          match(selected_loans$BorrowerState, 
                                                state.abb)])
...
##################################################################################
# Esse bloco cria dois mapas, um para bons e outro para mal pagadores, foi       #
# utilizado azul para indicar os bons pagadores e vermelho para os inadimplentes.#
##################################################################################

#mapa de inadimplentes
ggplot() + 
geom_map(data = map, map = map,
              aes(x = long, y = lat, map_id = region)) +
geom_map(data =  subset(selected_loans.fc_by_state,
                        selected_loans.fc_by_state$inadimplente == TRUE), 
...
#mapa de bons pagadores
ggplot() + 
geom_map(data = map, map = map,
          aes(x = long, y = lat, map_id = region)) +
geom_map(data =  subset(selected_loans.fc_by_state, 
                        selected_loans.fc_by_state$inadimplente == FALSE), 
              aes(fill = n, map_id = stateFullName),
              map = map) + 
     expand_limits(x = map$long, y = map$lat) +
     coord_map() +
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#cria um agrupamento por categoria do empréstimo
selected_loans.fc_by_ListingCategory <- selected_loans %>%
  group_by(ListingCategory) %>%
  summarise(n = n()) 

#labels dos empréstimos na ordem que serão exibidos
...
#Responsável por exibir um gráfico de barras das categorias dos empréstimos
ggplot(selected_loans.fc_by_ListingCategory, aes(x = ListingCategory, 
                                                 y = n, label = n)) +
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Responsável por exibiro histogramas da renda mensal de acordo com a situação
#do mutuário.
ggplot(aes(x = StatedMonthlyIncome, fill = inadimplente), 
       data = selected_loans) +
  geom_histogram() +
  scale_x_continuous(limits = c(0, 
                                quantile(selected_loans$StatedMonthlyIncome, 
                                         0.99)),
                     breaks = seq(0, 
                                  quantile(selected_loans$StatedMonthlyIncome, 
                                           0.99), 2500)) +
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Responsável por exibir um gráfico de dispersão entre o valor do empréstimo e o 
#retorno estimado
ggplot(aes(x = LoanOriginalAmount, y = EstimatedReturn), 
  ...
  scale_y_continuous(limits = c(0, 0.3), labels = scales::percent) +
   labs(title="Valor do empréstimo por retorno estimado",
        x = "Valor do Empréstimo",
        y = "Retorno Estimado")
#verifica a correlçao entre o valor do empréstimo e o retorno estimado
...
#verifica a correlçao entre o valor do empréstimo e o número de investidores
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Responsável por exibir um boxplot das situações dos empréstimos.
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#define a ordem que os scores serão exibidos
scores <-c("AA", "A", "B", "C", "D", "E", "HR")

#Responsável por exibir um BoxPlot dos scores de risco por taxa de júros
# dos empréstimos.
box_vc <- ggplot(aes(x = ProsperRating, y = BorrowerRate, 
                     label = ProsperRating), data = selected_loans) +
  geom_boxplot() + 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#cria um agrupamento dos mutuários inadimplentes
inadimplentes_by_BCUtilization <- group_by(
                                          subset(selected_loans,
                                          selected_loans$inadimplente == TRUE),
                                          ProsperRating)
#obtém o valor médio da utilização do crédito rotativo dos inadimplentes
inadimplentes_mp_by_BCUtilization <- summarise(
                                              inadimplentes_by_BCUtilization,
                                              mean_price = mean(BankcardUtilization))
#cria um agrupamento dos mutuários bons pagadores
loans_bons_by_BCUtilization <- group_by(
                                        subset(
                                            selected_loans, 
                                            selected_loans$inadimplente == FALSE),
                                            ProsperRating)
#obtém o valor médio da utilização do crédito rotativo dos bons pagadores
bons_mp_by_BCUtilization <- summarise(loans_bons_by_BCUtilization, 
                                      mean_price = mean(BankcardUtilization))

##################################################################################
#Bloco responsável por gerar os gráficos de barras dos mutuários inadimplentes   #
# e dos bons pagadores, onde o eixo X é o score de risco e o y é o uso do crédito#
# rotativo do cartão de crédito                                                  #
##################################################################################
p1 <- ggplot(aes(x = ProsperRating, y = mean_price),
             data = inadimplentes_mp_by_BCUtilization) +
...
p2 <- ggplot(aes(x = ProsperRating, y = mean_price),
             data = bons_mp_by_BCUtilization) +
        ...
        labs(title = "Média de uso do cartão de crédito de mutuários bons pagadores", 
             y = "Uso cartão de crédito",
             x = "Score de Risco")
#Responsável por exibir os gráficos gerados em uma grid
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Responsável por exibir um gráfico de dispersão, onde o eixo X corresponde aos 
#dias em atraso, o eixo Y corresponde ao usuo do cartão de crédito e as cores
# correspondem a situação de emprego do mutuário.
ggplot(aes(x = LoanCurrentDaysDelinquent, y = BankcardUtilization, 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Responsável por exibir o rendimento do credor de acordo com a situação do
#mutuário(Inadimplente ou Bom Pagador).
#A visualização foi facetada pela situação de emprego do mutuário.
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Define matizes igualmente espaçadas. 
gg_color_hue <- function(n) {hcl(h=seq(15, 375, length=n+1), l=65, c=100)[1:n]}
#Exibe um gráfico de linhas onde o eixo X corresponde ao ano do pagamento dos
#empréstimos, o eixo Y ao número de pagamentos em dia e as cores a faixa de
#renda dos mutuários.
ggplot(aes(x = YearLoan,  y = OnTimeProsperPayments), 
  ...
  scale_color_manual(
    "Faixa de Renda",
    values=c(gg_color_hue(length(unique(selected_loans$IncomeRange))),
             "orange", "darkred")
  ) +
   labs(title="Pagamentos em dia por faixa de renda", 
     y = "Pagamentos em dia",
     x = "Ano do pagamento")
```
...
#Exibe o rendimento do credor em cada estado dos EUA. 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Cria um agrupamento pela categoria do empreéstimo e por situação
selected_loans.fc_by_Category_inadimplente <- selected_loans %>%
  group_by(ListingCategory, inadimplente) %>%
  summarise(n = n()) 

#Responsável por exibir um gráfico de barras das categorias de empréstimo, separado
#pela situação do mutuário.
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Responsável por exibir um gráfico de dispersão onde o eixo X corresponde ao
#valor do empréstimo, o Y ao retorno estimado e as cores ao Score de Risco dos
#mutuários.
ggplot(aes(x = LoanOriginalAmount, y = EstimatedReturn, color = ProsperRating), 
  ...
  scale_color_brewer(type = 'div',
   ...
   labs(title="Retorno Estimado por Montante de Empréstimo",
        x = "Valor do Empréstimo",
        y = "Retorno Estimado")
#Exibe a correlação entre o valor do empréstimo e o retorno estimado.
with(selected_loans, cor.test(LoanOriginalAmount, EstimatedReturn))
...
Acredito que o trabalho foi interessante, com grandes descobertas em varios pontos, mas sei que ficaram alguns pontos sem explicação devido a quantidade de variáveis analisadas, pouco tempo e falta de informações importantes como: estado civil, sexo, situação de emprego do cônjugue, ou seja, informações mais pessoais dos mutuários. Para trabalhos futuros, penso em fechar o estudo com as variáveis que não foram exploradas, a fim de desenvolver alguns modelos de regressão para tentar prever quando uma solicitação de empréstimo é um bom investimento.  
